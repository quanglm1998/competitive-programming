...
int32_t main() {
  ...
  vector<vector<int>> min_val(n, vector<int>(n, 0));
  for (int i = 0; i < n; i++) {
    min_val[i][i] = h[i];
    for (int j = i + 1; j < n; j++) {
  ...
  auto Check = [&](int u, int v) {
    int min_h = u < v ? min_val[u][v] : min_val[v][u];
    return min_h == h[v];
  };

  vector<Mint> pref_dp(n, Mint(1));
  for (int i = 0; i < n; i++) {
    vector<Mint> nxt_pref_dp(n);
    for (int j = 0; j < n; j++) {
      if (!Check(i, j)) continue;
      nxt_pref_dp[j] = pref_dp[j];
    }
    for (int j = 1; j < nxt_pref_dp.size(); j++) {
      nxt_pref_dp[j] += nxt_pref_dp[j - 1];
    }
    swap(pref_dp, nxt_pref_dp);
  }
  cout << pref_dp.back() << '\n';
  return 0;
}